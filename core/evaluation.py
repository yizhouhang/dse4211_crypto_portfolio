...
# ============================================================
def normalize_strategy_table(df):
    out = df.copy()
    ...
    if "Log_Return" in out.columns and "log_return" not in out.columns:
        rename_map["Log_Return"] = "log_return"
    if rename_map:
...
def prepare_post_trade_df(
    ...
    out = normalize_strategy_table(post_trade_df)
...
# ============================================================
# 3) Daily marked-to-market evaluation
# ============================================================
...
# ============================================================
# 5) Build trade log from position series
# ============================================================
...
def _finalise_plot(output_path=None, show=True):
    if output_path:
...
def plot_equity_curve(
    ...
    plt.tight_layout()
    _finalise_plot(output_path=output_path, show=show)
...
def plot_cumulative_pnl(
    ...
    plt.figure(figsize=(12, 5))
    plt.plot(df[date_col], df["cumulative_pnl_series"])
    plt.title(title)
    plt.xlabel("Date")
